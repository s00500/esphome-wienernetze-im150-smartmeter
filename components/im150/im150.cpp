...
namespace esphome
{
    namespace im150
    {
        ...
        void IM150::handle_message(std::vector<uint8_t> msg)
        {
            ...
            // Decrypt
            uint8_t msglen = datalen - 31;
            uint8_t message[msglen] = {0};
            memcpy(message, msg.data() + 28, msglen);

            uint8_t nonce[16] = {0};
            memcpy(nonce, msg.data() + 14, 8);
            memcpy(nonce + 8, msg.data() + 24, 4);
            nonce[15] = 0x02;
            this->ctraes128.setKey(this->key, 16);
            this->ctraes128.setIV(nonce, 16);
            this->ctraes128.decrypt(message, message, msglen);
            ESP_LOGV(TAG, "Nonce: %s", format_hex_pretty(std::vector<uint8_t>(nonce, nonce + 16)).c_str());
